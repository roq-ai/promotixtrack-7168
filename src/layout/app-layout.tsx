...
import { getDigitalMarketingAgencies } from 'apiSdk/digital-marketing-agencies';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<DigitalMarketingAgencyInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/digital-marketing-agencies` : null),
    () => getDigitalMarketingAgencies({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
...

    {
      ...
      path: isTenantUser ? `/digital-marketing-agencies/view/${data?.[0]?.id}` : '/digital-marketing-agencies',
